...
export CONFIG_SRC="$HOME/dotfiles/zsh/zshrc.org"
...
alias refresh="tangle-file $CONFIG_SRC &> /dev/null && stow zsh -d $HOME/dotfiles && source $CONFIG_FILE && echo 'Refreshed config'"
...
** What is holding onto a port?
#+begin_src bash
function whomport() { lsof -nP -i4TCP:$1 | grep LISTEN }
#+end_src

** Tangle an org-mode file
#+begin_src bash
